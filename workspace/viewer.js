// workspace/viewer.js
// "나의 폴더" 개인 책 뷰어 — subjects/archives 뷰어와 완전히 같은 페이지 삽입/필기/
// 레이어/동기화/PWA 엔진(shared/annotation-engine.js)을 그대로 쓴다. 차이는 딱 하나:
// 이 뷰어가 여는 책은 PDF 원본이 없다(pdfDoc은 항상 null로 남는다) — 모든 페이지가
// 처음부터 끝까지 기존 "페이지 삽입" 기능(blank/lined/grid + 이번에 추가한 photo)으로만
// 만들어진다. bookId는 workspace:books의 book.id(personal_ 접두어)를 그대로 쓴다.
// 자료실/과목 뷰어처럼 "자료실에서 고르기" 화면 자체가 없다 — 항상 ?book=<id>로 특정
// 책 하나를 곧장 연다(폴더 안 책 목록은 workspace/folder.html이 담당).
...
let bookId = null, currentFolderId = null, currentBookTitle = '';
...
function pageLabel(pos) {
  ...
  return entry ? (entry.label || '새 페이지') : ('' + pos);
}
// entry가 없는 경우(예: 페이지가 아직 하나도 없는 새 책에서 처음 삽입할 때 afterPos=0
// 을 넘겨 받으면 getOrderEntry(1)이 없음)에도 페이지 삽입 모달이 참조할 기본 크기가
// 있어야 한다 — pdfDoc이 없는(=이 뷰어에서는 항상 없는) 책에서 크래시 없이 A4 비율로
// 대체한다. subjects/archives 뷰어는 이 분기를 절대 타지 않는다(항상 pdfDoc이 있음).
async function getPageViewport1(pos) {
  const entry = Engine.PageOrder.getOrderEntry(pos);
  if (entry && entry.kind === 'inserted') return { width: entry.width, height: entry.height };
  if (!pdfDoc) return DEFAULT_PAGE_SIZE;
  const pg = await pdfDoc.getPage(entry ? entry.pdfPage : pos);
  return pg.getViewport({ scale: 1 });
}

/* ══ 책 열기 — 항상 ?book=<id>로 특정 책 하나를 곧장 연다(자료실 선택 화면 없음) ══ */
// 책을 여는 동안 이전 화면이 남아 보이지 않게 한다 — 페이지만 넘기는 renderPages()에는
// 절대 안 쓰고, "책 자체가 바뀌는" 진입점에서만 부른다.
...
async function openWorkspaceBook(bid, folderId, title) {
  releaseCurrentLock();
  bookId = bid; currentFolderId = folderId || null; currentBookTitle = title || '(제목 없음)';
  document.title = currentBookTitle + ' — 나의 폴더';
  document.getElementById('header-book-title').textContent = currentBookTitle;
  showBookLoadingOverlay();
  try {
    Engine.setBook(bookId);
    // pdfNumPages=0 — 이 책은 PDF 원본이 없다. PageOrder.init()은 이미 0페이지를
    // 정상 지원한다(로컬에 저장된 삽입 페이지가 있으면 그걸, 없으면 빈 배열을 씀).
    const { totalPages } = await Engine.initPageOrder(bookId, 0);
    totalPg = totalPages; curPage = totalPages > 0 ? 1 : 0; zoom = 100;
    document.getElementById('sb-filename').textContent = currentBookTitle;
    Engine.Storage.recordRecent({ subjectId: 'workspace', bookId, title: currentBookTitle, viewerPath: CURRENT_VIEWER_PATH });
    await buildThumbs(); await renderPages(); updatePageInfo();
  } finally {
    hideBookLoadingOverlay(); // 첫 화면이 실제로 그려진(또는 0페이지 안내가 뜬) 뒤에야 가림을 걷는다
  }
}
// URL의 folder/title이 없을 때(예: 허브 "최근 학습" 카드로 들어온 경우 — 그 경로는
// book만 붙여서 이동시킨다)는 전체 책 목록에서 역으로 찾아 채운다.
async function initFromUrl() {
  const params = new URLSearchParams(location.search);
  const bid = params.get('book');
  if (!bid) { showToast('열 자료 정보를 찾을 수 없어요'); return; }
  let folderId = params.get('folder'), title = params.get('title');
  if (!folderId || !title) {
    const books = await Engine.Storage.listBooks();
    const found = books.find(b => b.id === bid);
    if (found) { folderId = folderId || found.folderId; title = title || found.title; }
  }
  await openWorkspaceBook(bid, folderId, title);
}
function goToFolder() {
  releaseCurrentLock();
  location.href = currentFolderId ? ('folder.html?folder=' + encodeURIComponent(currentFolderId)) : 'folder.html';
}
...
async function buildThumbs() {
  ...
  for (let p = 1; p <= totalPg; p++) {
    ...
    if (isInserted) {
      ...
      const pg = await pdfDoc.getPage(entry ? entry.pdfPage : p);
...
function openToolbarInsertMenu(e) {
  if (!bookId) { showToast('먼저 자료를 열어주세요'); return; }
...
// 페이지가 아직 하나도 없을 때(totalPg===0, curPage===0)는 "앞에"/"뒤에"가 의미가
// 없다 — 둘 다 afterPos=0(맨 앞=유일한 위치)으로 취급한다. 그렇지 않으면 curPage-1이
// -1이 되어 order.splice(-1,...)가 "맨 끝 바로 앞"에 끼워 넣는 엉뚱한 동작을 한다.
function toolbarInsertPage(where) {
  closeToolbarInsertMenu();
  openPageInsertModal(totalPg === 0 ? 0 : (where === 'before' ? curPage - 1 : curPage));
}
...
// 사진 페이지는 다른 3종과 달리 "파일이 있어야 페이지를 만들 수 있다" — 그래서
// 흐름이 다르다: 파일 선택 → 온라인 확인 → 압축 → 업로드(끝날 때까지 기다림) →
// 업로드가 성공한 다음에만 insertPages()로 실제 페이지를 만든다. 레이어의
// dataURL→서버경로 치환(migrateDataUrlAssets)처럼 "일단 로컬에 넣고 나중에 몰래
...
function rotateCurrentPhotoPage() {
  const entry = Engine.PageOrder.getOrderEntry(curPage);
  if (!entry || entry.kind !== 'inserted' || entry.template !== 'photo') { showToast('사진 페이지에서만 회전할 수 있어요'); return; }
  Engine.PageOrder.rotatePhotoPage(curPage);
  showToast('🔄 사진을 회전했어요');
}
...
async function renderPages() {
  ...
  // 자료실/과목 뷰어는 절대 안 타는 분기 — 이 뷰어가 여는 책은 PDF가 없어서 totalPg가
  // 정말 0에서 시작할 수 있다. "페이지가 아직 없습니다" 안내 + 바로 삽입 버튼을 보여줘서
  // 사용자가 상단 툴바의 작은 ➕📄 버튼을 굳이 안 찾아도 되게 한다.
  if (!totalPg) {
    main.innerHTML = '<div class="ws-empty-page"><div class="ws-empty-icon">📄</div><div class="ws-empty-title">아직 페이지가 없어요</div><div class="ws-empty-sub">아래 버튼으로 첫 페이지를 추가해보세요</div><button class="dlg-btn primary" onclick="openPageInsertModal(0)">➕ 페이지 추가</button></div>';
    updatePageInfo();
    return;
  }
...
async function computeFitZoom(mode) {
  if (!totalPg) return zoom; // subjects/archives 뷰어는 pdfDoc 유무로 판단하지만, 이 뷰어는 PDF가 없으므로 대신 페이지 존재 여부로 판단한다
...
function initSidebarResizer() {
  ...
  function endDrag() {
    ...
    if (totalPg) { zoom = Math.max(25, Math.min(400, Math.round(sidebarWidth / SIDEBAR_BASE_W * 100))); document.getElementById('vt-zoom').textContent = zoom + '%'; renderPages(); }
  }
...
function renderBookmarkList() {
  ...
  const map = []; // 이 뷰어가 여는 책은 PDF 원본이 없어 PAGE_MAPS 같은 목차 메타데이터가 없다 — 항상 아래 else(삽입 페이지 목록) 분기로 렌더링한다.
  if (map.length) {
    ...
    for (let i = 1; i <= totalPg; i++) {
      ...
      item.innerHTML = `<div class="bm-lbl">${escapeHtml(pageLabel(i))}</div><div class="bm-pg">${i}p</div>`;
...
const Sync = (function () {
  ...
  async function showConflictPreview(which) {
    ...
    let scale = 0.15;
    // getPageViewport1()은 pdfDoc 없이도(entry.width/height 또는 DEFAULT_PAGE_SIZE로)
    // 항상 값을 돌려주므로, 이 뷰어에서는 subjects/archives처럼 pdfDoc 유무를 따로
    // 검사할 필요가 없다 — 항상 시도한다.
    try { const vp = await getPageViewport1(pos); scale = canvas.width / vp.width; } catch (e) { }
    layers.forEach(l => {
...
// 구성은 이미 localStorage에 로컬 우선으로 저장되므로 별도 프리캐시가 필요 없다.

/* ══ 허브 이동 ═════════════════════════════════════════════ */
// 과목 전환기 자리에는(과목 뷰어와 달리) "← 폴더로" 버튼을 둔다 — goToFolder() 참고.
function goHome() { releaseCurrentLock(); location.href = '../index.html'; }

/* ══ 토스트 ═══════════════════════════════════════════════ */
// 페이지 이름표(entry.label)는 사용자가 자유 입력한 텍스트라, innerHTML로 꽂는
// 곳(썸네일 목록·목차)에서는 반드시 이 함수로 이스케이프한다. textContent로만
// 넣는 곳(페이지 상단 이름표 등)은 애초에 안전해서 이 함수가 필요 없다.
...
document.addEventListener('keydown', e => {
  ...
  if (!bookId || !totalPg) return; // subjects/archives 뷰어는 pdfDoc 유무로 판단하지만, 이 뷰어는 PDF가 없으므로 대신 책/페이지 존재 여부로 판단한다
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initFromUrl(); // ?book=<id>(&folder=&title=)로 특정 책을 곧장 연다
});
