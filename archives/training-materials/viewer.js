// archives/training-materials/viewer.js
// 교사 연수 자료 뷰어 — UI/DOM 담당. 필기/레이어/동기화/PWA 로직은 전부 AnnotationEngine에 위임한다.
// bookId는 항상 PDF 파일명(확장자 제외) — bookIdOf()는 training-materials-data.js에 정의됨.
// subjects/*의 과목 뷰어와 완전히 같은 패턴을 그대로 재사용한다 — 차이는 "단계" 필터를
// FILTER_LABEL(카테고리별 라벨, training-materials-data.js에 정의)로 일반화한 것뿐이다.

pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';

const CACHE_NAME = 'kth-content-v1'; // sw.js의 CONTENT_CACHE와 반드시 동일해야 함(모든 과목/자료실 공용 캐시 버킷)
const CURRENT_SUBJECT_ID = 'training-materials'; // archives.json의 id와 일치 — 자료실 전환기 기본 선택, 최근 학습 기록에 쓰임
const CURRENT_VIEWER_PATH = 'archives/training-materials/viewer.html';
...
// MATERIALS에 있는 stage 값들로 필터 옵션을 자동 생성한다 — 카테고리마다 필터 체계가
// 달라도(연도, 발행호, 주제 등 — training-materials-data.js의 FILTER_LABEL) 이 함수는
// 손댈 필요 없이 그대로 재사용된다. 자료관리 화면의 필터 드롭다운(ms-stage-filter)과
// 뷰어 상단의 필터 드롭다운(vt-stage-filter) 둘 다 이 함수 하나로 채운다 — 옵션 목록을
// 두 곳에서 따로 만들지 않는다.
...
function populateStageFilter() {
  ...
  const optionsHtml = `<option value="all">전체 ${FILTER_LABEL}</option>` + stages.map(s => `<option value="${s}">${s}</option>`).join('');
  STAGE_FILTER_SELECT_IDS.forEach(id => { const sel = document.getElementById(id); if (sel) sel.innerHTML = optionsHtml; });
  document.querySelectorAll('[data-filter-label-title]').forEach(el => { el.title = `${FILTER_LABEL}별 자료 목록으로 이동`; });
}
function renderList() {
  ...
  list.innerHTML = items.map(m => `
    <div class="ms-item">
      ...
      <div class="ms-item-info">
        <div class="ms-item-subject">${m.stage ? m.stage + ' · ' : ''}${m.label}</div>
...
async function openMaterial(file) {
  ...
  try {
    ...
    showToast('PDF 로딩 중...');
    let loadingTask;
    // Range Request로 필요한 페이지만 부분적으로 받는다 — 대형 PDF(수백 페이지)로
...
async function afterPdfLoaded(doc, label) {
  ...
  // 지금 연 자료의 필터 값으로 뷰어 상단 드롭다운도 맞춰준다(커스텀 업로드 파일은 값이 없음).
...
// 뷰어 상단 필터 드롭다운 — 자료관리 화면과 완전히 같은 필터 select(ms-stage-filter)에
...
function buildPrecacheUrls() {
  const urls = ['../../index.html', '../../subjects.json', '../../archives.json', '../../manifest.json', '../../shared/annotation-engine.js', './viewer.html', './viewer.js', './training-materials-data.js'];
...
/* ══ 자료실 전환기 / 허브 이동 ═════════════════════════════ */
// subjects.json이 아닌 archives.json을 읽는다 — 이 뷰어는 자료실 카테고리 소속이라
// 전환기에서 나머지 4개 자료실 카테고리끼리 오가는 게 자연스럽다(과목 전환은 🏠 허브로
// 나가서 하면 된다).
function goHome() { releaseCurrentLock(); location.href = '../../index.html'; }
async function initSubjectSwitcher() {
  try {
    const res = await fetch('../../archives.json', { cache: 'no-store' });
