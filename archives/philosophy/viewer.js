// archives/philosophy/viewer.js
// 구몬 교육 원론 뷰어 — UI/DOM 담당. 필기/레이어/동기화/PWA 로직은 전부 AnnotationEngine에 위임한다.
// bookId는 항상 PDF 파일명(확장자 제외) — bookIdOf()는 philosophy-data.js에 정의됨.
// subjects/*의 과목 뷰어와 완전히 같은 패턴을 그대로 재사용한다 — 차이는 "단계" 필터를
// FILTER_LABEL(카테고리별 라벨, philosophy-data.js에 정의)로 일반화한 것뿐이다.

pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';

const CACHE_NAME = 'kth-content-v1'; // sw.js의 CONTENT_CACHE와 반드시 동일해야 함(모든 과목/자료실 공용 캐시 버킷)
const CURRENT_SUBJECT_ID = 'philosophy'; // archives.json의 id와 일치 — 자료실 전환기 기본 선택, 최근 학습 기록에 쓰임
const CURRENT_VIEWER_PATH = 'archives/philosophy/viewer.html';
...
// 달라도(연도, 발행호, 주제 등 — philosophy-data.js의 FILTER_LABEL) 이 함수는
...
function buildPrecacheUrls() {
  const urls = ['../../index.html', '../../subjects.json', '../../archives.json', '../../manifest.json', '../../shared/annotation-engine.js', './viewer.html', './viewer.js', './philosophy-data.js'];
