// subjects/math/viewer.js
// 수학 뷰어 — UI/DOM 담당. 필기/레이어/동기화/PWA 로직은 전부 AnnotationEngine에 위임한다.
// bookId는 항상 PDF 파일명(확장자 제외) — bookIdOf()는 math-data.js에 정의됨.

pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';

const CACHE_NAME = 'kth-content-v1'; // sw.js의 CONTENT_CACHE와 반드시 동일해야 함(모든 과목 공용 캐시 버킷)
const CURRENT_SUBJECT_ID = 'math'; // subjects.json의 id와 일치 — 과목 전환기 기본 선택, 최근 학습 기록에 쓰임
const CURRENT_VIEWER_PATH = 'subjects/math/viewer.html';
...
function buildPrecacheUrls() {
  const urls = ['/index.html', '/subjects.json', '/manifest.json', '/shared/annotation-engine.js', '/subjects/math/viewer.html', '/subjects/math/viewer.js', '/subjects/math/math-data.js'];
